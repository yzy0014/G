...
# 50% cases from pop1; rest cases and all controls from pop2
# 125 pop1, 375 pop2

# nsnps=50 
# nind=500
# n1=125 		## total number of population 1
...
# a=0.5 		## proportion of cases from pop1

case_2<-function(nsnps, nind, n1, nd, R, a){
...
##p<-rep(runif(1,0.01,maffilter), nsnps)
...
nd1=n1 		## number of cases from pop1
nc1=0 		## number of controls from pop1
vec1=rep(1, n1)

p<-rep(.80, nsnps)					## allele frequencies
freq<-sapply(1:length(p),function(x) rbeta(1,p[x]*(1-fst)/fst,(1-p[x])*(1-fst)/fst))

## sample all cases from pop1
pop1<-sapply(1:nsnps, function(x) sample(c(0,1,2), nd1, replace=TRUE, prob=c(((1-freq[x])^2),(2*R*freq[x]*(1-freq[x])),(freq[x]^2*R^2)/sum((1-freq[x])^2,2*R*freq[x]*(1-freq[x]),R^2*freq[x]^2))))  ## sample cases
...
vec2=rep(0, n2)
vec2[sample(1:n2, size=nd2, replace = FALSE)]=1  ## cases/control status of pop1
