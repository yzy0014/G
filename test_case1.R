...
test_case1 <- function(num_loop, relative_risk, alpha, nsnps, nind, n1, nd, a) {
...
##
###########################
dataset <- case_1(nsnps, nind, n1, nd, R, a)
pop = dataset$dt
...
while(flag){
	...
	flag=length(which(ls==FALSE))
		...
		## logistic PCA
#########################    
lpca = logisticPCA(binary_pop(pop), k = 2, m = 2, main_effects = TRUE)
		x.lpca = lpca$PCs
...
Poly_cor <- polychoric(pop)$rho
		...
		# continous PCA
#########################
x.pca <- flashpca(scale2(pop), ndim=1, stand="none")$projection
...
## print(j)
			...
			chi_pca[i, j] = pvalue(independence_test(l.pca ~ SNP.pca, teststat = "quad")) ## l
			chi_lpca[i, j] = pvalue(independence_test(l.lpca ~ SNP.lpca, teststat = "quad")) ## l
			chi_poly[i, j] = pvalue(independence_test(l.poly ~ SNP.poly, teststat = "quad")) ## l
